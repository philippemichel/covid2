...
mongg <- function(dd,
  ...
  aes(x = {{dx}}, y = {{dy}}) +
  geom_line(size = 0.4) +
  geom_smooth(span = 0.15, col = colx) +
...
zz <- tt %>% 
  dplyr::filter(dep == 95)
titre <- paste0("Val d'Oise (",dj,") - admissions en réanimation")

mongg(tail(zz,122), dy = incid_hosp, tit = titre, stit ="trois derniers mois",colx ="red")
