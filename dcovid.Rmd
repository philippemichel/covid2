...
```{r graphFr, warning=FALSE, fig.width=20, fig.height=4}
zzf <- tt %>%
  group_by(jour) %>%
...
mongg(zzf,
  tit = "France métropolitaine -  nouvelles hospitalisations",
...
```{r graphFr2, warning=TRUE, fig.width=22, fig.height=7}

mongg(tail(zzf, 122),
  tit = "France métropolitaine - Trois derniers mois",
...
```{r mhebdo, fig.width=22, fig.height=7}
tz <- NULL
dz <- NULL
zz <- tt %>%
  group_by(jour) %>%
